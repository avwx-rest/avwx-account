...
def activate(plan: str):
    ...
    if request.method == 'POST':
        ...
        flash(f'Your {plan} plan is now active. Thank you for supporting AVWX. You can now generate your API token', 'success')
        return redirect('manage')
    else:
...
def change(plan: str):
    ...
    if request.method == 'POST':
        ...
        flash(f'Your {plan} plan is now active. Thank you for your continued support!', 'success')
        return redirect('manage')
...
def generate_token():
    if current_user.apitoken and not current_user.active_token:
        ...
        flash('Your API token could not be generated. Do you have an active subscription?', 'error')
    return redirect('manage')
